    ...
    const prioritizeAndRun = (conditions, operator) => {
      ...
      for (let i = 0; i < orderedSets.length; i++) {
        ...
        cursor = cursor.then((setResult) => {
          // After the first set succeeds or fails, don't fire off the remaining promises
          if ((operator === 'any' && setResult === true) || (operator === 'all' && setResult === false) || stop) {
            debug(
              `prioritizeAndRun::detected ${setResult ? 'truthy' : 'falsey'} result; skipping remaining conditions`
            );
            stop = true;
            return setResult;
          }
